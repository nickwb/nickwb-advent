...
impl Direction {
    fn project(&self, from: &Point, distance: Grid) -> Point {
        match self {
            Direction::Up => pt(from.x, from.y + distance),
            Direction::Down => pt(from.x, from.y - distance),
            Direction::Left => pt(from.x - distance, from.y),
            Direction::Right => pt(from.x + distance, from.y),
        }
    }
}
