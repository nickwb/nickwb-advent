...
fn parse_bodies(lines: &[&str]) -> Bodies {
...
fn count_orbits(bodies: &Bodies) -> usize {
    let mut ancestors: HashMap<&str, usize> = HashMap::new();
    let mut total: usize = 0;

    ancestors.insert("COM", 1usize);

    for b in bodies.iter() {
        let parent_key: &str = &b.parent;
        let my_key: &str = &b.label;

        if !ancestors.contains_key(&parent_key) {
            panic!("Not seen: {}", parent_key);
        }

        let parent_count = ancestors[parent_key];
        ancestors.insert(my_key, parent_count + 1);
        total += parent_count;
    }
...
pub fn run_day_six() {
    let bodies = parse_bodies(&MY_INPUTS);
    println!("Day 5, Part 1: {}", count_orbits(&bodies));
}
...
fn example_1() {
    ...
    assert_eq!(42, count_orbits(&bodies));
}
